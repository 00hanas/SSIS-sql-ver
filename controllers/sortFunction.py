...
def sortTable(tableWidget: QTableWidget, column_name: str, ascending: bool):

    # Get primary sort column index
    primary_col = getColumnIndex(tableWidget, column_name)

    # Determine the correct secondary column 
    possible_secondary_columns = ["First Name", "Program Code"]
    secondary_col = next((getColumnIndex(tableWidget, col) for col in possible_secondary_columns if getColumnIndex(tableWidget, col) is not None), None)
...
def getColumnIndex(tableWidget: QTableWidget, column_name: str):
